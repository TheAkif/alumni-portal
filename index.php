...
<?php
	...
	if(isset($_POST['login_submit'])){
		...
		$university=$_POST['uni_login'];

		$connection = new mysqli("localhost","root","",$university);

		$email=mysqli_real_escape_string($connection,$_POST['email_login']);
		$pass=mysqli_real_escape_string($connection,$_POST['password_login']);
		$pass = md5($pass);
		$role=$_POST['option'];

		$q = "SELECT * from ".$role." WHERE email_address='".$email."' AND password='".$pass."'"; 
		$rows = $connection->query($q); 
		

		$trow = $rows->num_rows;
		if ( $trow > 0) {

			$_SESSION['ssuniversity'] = $university;
			

			while ($result= $rows->fetch_assoc()) {
			if ($role == 'admin') {
			 	$_SESSION['ssid'] = $result['admin_id'];
			 }
			 elseif ($role == 'representative') {
			  	$_SESSION['ssid'] = $result['rep_id'];
			  } 
			  else{
			  	$_SESSION['ssid'] = $result['alumni_id'];
			  }
			}

			$_SESSION['ssemail'] = $email;
			$_SESSION['ssrole'] = $role;
 		}
 		else{
 			$error=true;
 		}
 		...
 		if (isset($_SESSION['ssuniversity'])) {
 			header("Location: dashboard/index.php");
 		}
	...
	if(isset($_POST['signup_submit'])){
		...
		//total rows\
		$countrows1 = "SELECT count(*) AS COUNT from approve_to_portal";
		$rows1 = $connection->query($countrows1);
		$rowcount1 =$rows1->fetch_assoc();
		$rowcount1['COUNT']++;

		$countrows2 = "SELECT count(*) AS COUNT from approval_data";
		$rows2 = $connection->query($countrows2);
		$rowcount2 =$rows2->fetch_assoc();
		$rowcount2['COUNT']++;

		$insert1 = "INSERT INTO `approve_to_portal`(`alumni_id`, `password`, `email_address`, `batch`) VALUES (".$rowcount1['COUNT'].",'".$password."','".$email."','".$batch."')";
		$insert2 = "INSERT INTO `approval_data`(`id`, `first_name`, `last_name`, `date_of_birth`, `contact_no`, `photo`, `address`, `Approve_to_portal_alumni_id`) VALUES (".$rowcount2['COUNT'].",'".$fname."','".$lname."','".$dob."','".$contact."','NULL','".$add."',".$rowcount2['COUNT'].")";
		$connection->query($insert1);
		
		print_r($insert2);
		
		if (!$connection->query($insert2)) {
			echo "<script>alert('Query did not Execute');</script>";
		}
		else{
			echo "<script>alert('Your data has gone for analysis. You'll be shortly informed about approval or rejection! Thank You);</script>";	
								      ...
								      <!-- Login TAB  -->
								        ...
								        <form method="POST" action="index.php">
								          ...
								          <div class="md-form">
							                    ...
							                    <select name="uni_login" required="" class="form-control">
							                        <option required="" disabled="">Select one</option>
							                        <?php $uni = new university(); $uni->display();?>							                        
							                    </select>
							                    
							                    <label for="">Choose Your University name</label><br>
							                    <a onclick="uninotfound()" style="text-decoration:underline;">Didn't find your university here?</a>
							                </div>
								          <br>
...
<script type="text/javascript">
	...
	function uninotfound(){
		alert("Your University has not Registered for Alumni Portal Service yet. Tell them to Register ASAP!");
	}
